...
{
    // connection_deleter(const shared_ptr<Environment>& env) : env(env) {}
    connection_deleter(shared_ptr<Environment> env) : env(move(env)) {}
...
class db_manager 
{
  public:
    ...
    shared_ptr<Environment> get_environment() { return environment; }
    shared_ptr<Connection> get_connection() { return connection; }
    
    ResultSetSharedPtr query(const std::string& q) const
    {
	Statement *stmt= connection->createStatement(q);
	ResultSet *rs = stmt->executeQuery();
	return ResultSetSharedPtr{rs, result_set_deleter{connection, stmt} };
    }    
  private:
